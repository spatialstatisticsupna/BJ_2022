...
#results in terms of model selection criterion
for(mod in c(1:8)){
  load(paste0("Modelo",mod,".Rdata"))
  result=get(paste0("result",mod))

  DIC=result$dic$dic
  DIC_Inci=sum(result$dic$local.dic[1:1320], na.rm=T)
  DIC_Morta=sum(result$dic$local.dic[1321:2640], na.rm=T)
  
  pD=result$dic$p.eff
  pD_Inci=sum(result$dic$local.p.eff[1:1320], na.rm=T)
  pD_Morta=sum(result$dic$local.p.eff[1321:2640], na.rm=T)
  medidas2=cbind(DIC,DIC_Inci,DIC_Morta,pD,pD_Inci,pD_Morta)
  print(medidas2)
  assign(paste0("results",mod),medidas2)
}

### Results of Table 1 and Table 4 

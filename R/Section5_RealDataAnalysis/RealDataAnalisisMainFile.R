...
#rm(list=ls())
#setwd(".../R/Section5_RealDataAnalysis")
######################################################################
#library(tidyverse)
library(Hmisc)
library(classInt)
...
print(xtable(table3,caption ="Observed versus Predicted number of Brain Cancer incidence cases per period"), caption.placement = 'top')
...
fig7 <- ggplot(data = datplot, aes(x=year, y=ratet,color=Gender)) + 
...
pdf("Fig7.pdf",width = 15,height=7)
print(fig7)
dev.off()
...
tmap_save(tm,paste0("Fig8_",ag,".pdf"))
...
for(ag in 4:6){
  ...
  tm <- tm_shape(carto) + 
    ...
    tm_layout(main.title="",
              ...
              frame = FALSE) +
    tm_facets(nrow=1, ncol=2)
  tmap_save(tm,paste0("Fig9_",ag-3,".pdf"))
}



############################################################################
#### Figure 11 Appendix A.2 Coefficients of variation (CV) of the predicted rates by regions and age-groups obtained
#as the posterior standard deviation of the rates divided by the posterior mean. 
...
tmap_save(cvmap,paste0("Fig11.pdf"))
